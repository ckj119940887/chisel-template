...
class AXI4LiteSlaveDDR4Simplified(val C_S_AXI_ADDR_WIDTH: Int, 
  ...
  withReset(lowActiveReset) {
    ...
    // r_control(2)  -- readData    
    // r_control(3)  -- readValid   
    // r_control(4)  -- writeAddr  
    // r_control(5)  -- writeData   
    // r_control(6)  -- writeValid  
    // r_control(7)  -- dmaSrcAddr   
    // r_control(8)  -- dmaDstAddr   
    // r_control(9)  -- dmaSrcLen    
    // r_control(10) -- dmaDstLen    
    // r_control(11) -- dmaValid     
    val r_control = Reg(Vec(12, UInt(64.W)))

    val axi4FullMaster = Module(new AXI4FullMasterSimplified(C_M_AXI_ADDR_WIDTH = 49, 
    ...
    when(axi4FullMaster.io.readValid) {
        r_control(2)               := axi4FullMaster.io.readData
        r_control(3)               := true.B
    }

    axi4FullMaster.io.writeAddr    := r_control(4) 
    axi4FullMaster.io.writeData    := r_control(5) 
    when(axi4FullMaster.io.writeValid) {
        r_control(6)              := true.B 
    }

    axi4FullMaster.io.dmaSrcAddr   := r_control(7) 
    axi4FullMaster.io.dmaDstAddr   := r_control(8) 
    axi4FullMaster.io.dmaSrcLen    := r_control(9) 
    axi4FullMaster.io.dmaDstLen    := r_control(10) 
