...
class TopOddEven(val numIPs: Int, val depth: Int, val addrWidth: Int, val dataWidth: Int) extends Module{
    ...
    val modEven = Module(new Even(addrWidth = addrWidth, dataWidth = dataWidth, stackDepth = 256, cpWidth = 5, idWidth = 2, depth = depth))
    val modOdd  = Module(new Odd(addrWidth = addrWidth, dataWidth = dataWidth, stackDepth = 256, cpWidth = 5, idWidth = 2, depth = depth))
    ...
    arbMem.io.ipReqs(0)    <> modEven.io.arbMem_req
    arbMem.io.ipReqs(1)    <> modOdd.io.arbMem_req
    modEven.io.arbMem_resp <> arbMem.io.ipResps(0)
    modOdd.io.arbMem_resp  <> arbMem.io.ipResps(1)
    ...
    switch(TopOddEvenCP) {
      is(0.U) {
        r_valid      := false.B
        TopOddEvenCP := Mux(r_start_pulse, 1.U, 0.U)
      }
      is(1.U) {
