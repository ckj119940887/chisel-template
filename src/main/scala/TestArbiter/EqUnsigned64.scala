...
class EqUnsigned64(val width: Int = 64) extends Module {
...
class EqUnsigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = UInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class EqUnsigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = Bool()
}


class EqUnsigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new EqUnsigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new EqUnsigned64ResponseBundle(dataWidth)))
}


class EqUnsigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new EqUnsigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new EqUnsigned64ResponseBundle(dataWidth)))
  val ip      = new EqUnsigned64IO(dataWidth)
}


class EqUnsigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new EqUnsigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new EqUnsigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new EqUnsigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new EqUnsigned64ResponseBundle(dataWidth)))
...
class EqUnsigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new EqUnsigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new EqUnsigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new EqUnsigned64(dataWidth))

    val r_req            = Reg(new EqUnsigned64RequestBundle(dataWidth))
...
class EqUnsigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new EqUnsigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new EqUnsigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new EqUnsigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new EqUnsigned64ResponseBundle(dataWidth))
  ...
  val EqUnsigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(Bool())

  switch(EqUnsigned64CP) {
    is(0.U) {
