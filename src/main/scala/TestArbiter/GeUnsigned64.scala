...
class GeUnsigned64(val width: Int = 64) extends Module {
...
class GeUnsigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = UInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class GeUnsigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = Bool()
}


class GeUnsigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new GeUnsigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new GeUnsigned64ResponseBundle(dataWidth)))
}


class GeUnsigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new GeUnsigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new GeUnsigned64ResponseBundle(dataWidth)))
  val ip      = new GeUnsigned64IO(dataWidth)
}


class GeUnsigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new GeUnsigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new GeUnsigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new GeUnsigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new GeUnsigned64ResponseBundle(dataWidth)))
...
class GeUnsigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new GeUnsigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new GeUnsigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new GeUnsigned64(dataWidth))

    val r_req            = Reg(new GeUnsigned64RequestBundle(dataWidth))
...
class GeUnsigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new GeUnsigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new GeUnsigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new GeUnsigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new GeUnsigned64ResponseBundle(dataWidth))
  ...
  val GeUnsigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(Bool())

  switch(GeUnsigned64CP) {
    is(0.U) {
