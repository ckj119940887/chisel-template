...
class OrSigned64(val width: Int = 64) extends Module {
...
class OrSigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = SInt(dataWidth.W)
  val b = SInt(dataWidth.W)
}


class OrSigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = SInt(dataWidth.W)
}


class OrSigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new OrSigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new OrSigned64ResponseBundle(dataWidth)))
}


class OrSigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new OrSigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new OrSigned64ResponseBundle(dataWidth)))
  val ip      = new OrSigned64IO(dataWidth)
}


class OrSigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new OrSigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new OrSigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new OrSigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new OrSigned64ResponseBundle(dataWidth)))
...
class OrSigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new OrSigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new OrSigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new OrSigned64(dataWidth))

    val r_req            = Reg(new OrSigned64RequestBundle(dataWidth))
...
class OrSigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new OrSigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new OrSigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new OrSigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new OrSigned64ResponseBundle(dataWidth))
  ...
  val OrSigned64CP            = RegInit(0.U(4.W))
