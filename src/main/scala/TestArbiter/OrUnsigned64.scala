...
class OrUnsigned64(val width: Int = 64) extends Module {
...
class OrUnsigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = UInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class OrUnsigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = UInt(dataWidth.W)
}


class OrUnsigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new OrUnsigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new OrUnsigned64ResponseBundle(dataWidth)))
}


class OrUnsigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new OrUnsigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new OrUnsigned64ResponseBundle(dataWidth)))
  val ip      = new OrUnsigned64IO(dataWidth)
}


class OrUnsigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new OrUnsigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new OrUnsigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new OrUnsigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new OrUnsigned64ResponseBundle(dataWidth)))
...
class OrUnsigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new OrUnsigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new OrUnsigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new OrUnsigned64(dataWidth))

    val r_req            = Reg(new OrUnsigned64RequestBundle(dataWidth))
...
class OrUnsigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new OrUnsigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new OrUnsigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new OrUnsigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new OrUnsigned64ResponseBundle(dataWidth))
  ...
  val OrUnsigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(UInt(dataWidth.W))

  switch(OrUnsigned64CP) {
    is(0.U) {
