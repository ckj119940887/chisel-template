...
class GtUnsigned64(val width: Int = 64) extends Module {
...
class GtUnsigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = UInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class GtUnsigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = Bool()
}


class GtUnsigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new GtUnsigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new GtUnsigned64ResponseBundle(dataWidth)))
}


class GtUnsigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new GtUnsigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new GtUnsigned64ResponseBundle(dataWidth)))
  val ip      = new GtUnsigned64IO(dataWidth)
}


class GtUnsigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new GtUnsigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new GtUnsigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new GtUnsigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new GtUnsigned64ResponseBundle(dataWidth)))
...
class GtUnsigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new GtUnsigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new GtUnsigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new GtUnsigned64(dataWidth))

    val r_req            = Reg(new GtUnsigned64RequestBundle(dataWidth))
...
class GtUnsigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new GtUnsigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new GtUnsigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new GtUnsigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new GtUnsigned64ResponseBundle(dataWidth))
  ...
  val GtUnsigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(Bool())

  switch(GtUnsigned64CP) {
    is(0.U) {
