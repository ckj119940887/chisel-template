...
class SubtractorSigned64(val width: Int = 64) extends Module {
...
class SubtractorSigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = SInt(dataWidth.W)
  val b = SInt(dataWidth.W)
}


class SubtractorSigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = SInt(dataWidth.W)
}


class SubtractorSigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new SubtractorSigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new SubtractorSigned64ResponseBundle(dataWidth)))
}


class SubtractorSigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new SubtractorSigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new SubtractorSigned64ResponseBundle(dataWidth)))
  val ip      = new SubtractorSigned64IO(dataWidth)
}


class SubtractorSigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new SubtractorSigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new SubtractorSigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new SubtractorSigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new SubtractorSigned64ResponseBundle(dataWidth)))
...
class SubtractorSigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new SubtractorSigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new SubtractorSigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new SubtractorSigned64(dataWidth))

    val r_req            = Reg(new SubtractorSigned64RequestBundle(dataWidth))
...
class SubtractorSigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new SubtractorSigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new SubtractorSigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new SubtractorSigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new SubtractorSigned64ResponseBundle(dataWidth))
  ...
  val SubtractorSigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(SInt(dataWidth.W))

  switch(SubtractorSigned64CP) {
    is(0.U) {
