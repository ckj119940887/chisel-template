...
class UshrUnsigned64(val width: Int = 64) extends Module {
...
class UshrUnsigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = UInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class UshrUnsigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = UInt(dataWidth.W)
}


class UshrUnsigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new UshrUnsigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new UshrUnsigned64ResponseBundle(dataWidth)))
}


class UshrUnsigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new UshrUnsigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new UshrUnsigned64ResponseBundle(dataWidth)))
  val ip      = new UshrUnsigned64IO(dataWidth)
}


class UshrUnsigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new UshrUnsigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new UshrUnsigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new UshrUnsigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new UshrUnsigned64ResponseBundle(dataWidth)))
...
class UshrUnsigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new UshrUnsigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new UshrUnsigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new UshrUnsigned64(dataWidth))

    val r_req            = Reg(new UshrUnsigned64RequestBundle(dataWidth))
...
class UshrUnsigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new UshrUnsigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new UshrUnsigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new UshrUnsigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new UshrUnsigned64ResponseBundle(dataWidth))
  ...
  val UshrUnsigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(UInt(dataWidth.W))

  switch(UshrUnsigned64CP) {
    is(0.U) {
