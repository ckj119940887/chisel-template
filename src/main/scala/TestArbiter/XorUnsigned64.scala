...
class XorUnsigned64(val width: Int = 64) extends Module {
...
class XorUnsigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = UInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class XorUnsigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = UInt(dataWidth.W)
}


class XorUnsigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new XorUnsigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new XorUnsigned64ResponseBundle(dataWidth)))
}


class XorUnsigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new XorUnsigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new XorUnsigned64ResponseBundle(dataWidth)))
  val ip      = new XorUnsigned64IO(dataWidth)
}


class XorUnsigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new XorUnsigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new XorUnsigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new XorUnsigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new XorUnsigned64ResponseBundle(dataWidth)))
...
class XorUnsigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new XorUnsigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new XorUnsigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new XorUnsigned64(dataWidth))

    val r_req            = Reg(new XorUnsigned64RequestBundle(dataWidth))
...
class XorUnsigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new XorUnsigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new XorUnsigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new XorUnsigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new XorUnsigned64ResponseBundle(dataWidth))
  ...
  val XorUnsigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(UInt(dataWidth.W))

  switch(XorUnsigned64CP) {
    is(0.U) {
