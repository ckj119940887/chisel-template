...
class GeSigned64(val width: Int = 64) extends Module {
...
class GeSigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = SInt(dataWidth.W)
  val b = SInt(dataWidth.W)
}


class GeSigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = Bool()
}


class GeSigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new GeSigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new GeSigned64ResponseBundle(dataWidth)))
}


class GeSigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new GeSigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new GeSigned64ResponseBundle(dataWidth)))
  val ip      = new GeSigned64IO(dataWidth)
}


class GeSigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new GeSigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new GeSigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new GeSigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new GeSigned64ResponseBundle(dataWidth)))
...
class GeSigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new GeSigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new GeSigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new GeSigned64(dataWidth))

    val r_req            = Reg(new GeSigned64RequestBundle(dataWidth))
...
class GeSigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new GeSigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new GeSigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new GeSigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new GeSigned64ResponseBundle(dataWidth))
  ...
  val GeSigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(Bool())
