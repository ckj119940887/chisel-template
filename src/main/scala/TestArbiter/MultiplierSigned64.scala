...
class MultiplierSigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = SInt(dataWidth.W)
  val b = SInt(dataWidth.W)
}


class MultiplierSigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = SInt(dataWidth.W)
}


class MultiplierSigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new MultiplierSigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new MultiplierSigned64ResponseBundle(dataWidth)))
}


class MultiplierSigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new MultiplierSigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new MultiplierSigned64ResponseBundle(dataWidth)))
  val ip      = new MultiplierSigned64IO(dataWidth)
}


class MultiplierSigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new MultiplierSigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new MultiplierSigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new MultiplierSigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new MultiplierSigned64ResponseBundle(dataWidth)))
...
class MultiplierSigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new MultiplierSigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new MultiplierSigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new MultiplierSigned64(dataWidth))

    val r_req            = Reg(new MultiplierSigned64RequestBundle(dataWidth))
...
class MultiplierSigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new MultiplierSigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new MultiplierSigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new MultiplierSigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new MultiplierSigned64ResponseBundle(dataWidth))
  ...
  val MultiplierSigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(SInt(dataWidth.W))

  switch(MultiplierSigned64CP) {
    is(0.U) {
