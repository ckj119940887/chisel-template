...
class XorSigned64(val width: Int = 64) extends Module {
...
class XorSigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = SInt(dataWidth.W)
  val b = SInt(dataWidth.W)
}


class XorSigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = SInt(dataWidth.W)
}


class XorSigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new XorSigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new XorSigned64ResponseBundle(dataWidth)))
}


class XorSigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new XorSigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new XorSigned64ResponseBundle(dataWidth)))
  val ip      = new XorSigned64IO(dataWidth)
}


class XorSigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new XorSigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new XorSigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new XorSigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new XorSigned64ResponseBundle(dataWidth)))
...
class XorSigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new XorSigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new XorSigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new XorSigned64(dataWidth))

    val r_req            = Reg(new XorSigned64RequestBundle(dataWidth))
...
class XorSigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new XorSigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new XorSigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new XorSigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new XorSigned64ResponseBundle(dataWidth))
  ...
  val XorSigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(SInt(dataWidth.W))

  switch(XorSigned64CP) {
    is(0.U) {
