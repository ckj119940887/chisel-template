...
class NeSigned64(val width: Int = 64) extends Module {
...
class NeSigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = SInt(dataWidth.W)
  val b = SInt(dataWidth.W)
}


class NeSigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = Bool()
}


class NeSigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new NeSigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new NeSigned64ResponseBundle(dataWidth)))
}


class NeSigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new NeSigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new NeSigned64ResponseBundle(dataWidth)))
  val ip      = new NeSigned64IO(dataWidth)
}


class NeSigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new NeSigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new NeSigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new NeSigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new NeSigned64ResponseBundle(dataWidth)))
...
class NeSigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new NeSigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new NeSigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new NeSigned64(dataWidth))

    val r_req            = Reg(new NeSigned64RequestBundle(dataWidth))
...
class NeSigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new NeSigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new NeSigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new NeSigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new NeSigned64ResponseBundle(dataWidth))
  ...
  val NeSigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(Bool())
