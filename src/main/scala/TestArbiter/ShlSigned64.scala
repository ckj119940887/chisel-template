...
class ShlSigned64(val width: Int = 64) extends Module {
...
class ShlSigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = SInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class ShlSigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = SInt(dataWidth.W)
}


class ShlSigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new ShlSigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new ShlSigned64ResponseBundle(dataWidth)))
}


class ShlSigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new ShlSigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new ShlSigned64ResponseBundle(dataWidth)))
  val ip      = new ShlSigned64IO(dataWidth)
}


class ShlSigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new ShlSigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new ShlSigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new ShlSigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new ShlSigned64ResponseBundle(dataWidth)))
...
class ShlSigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new ShlSigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new ShlSigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new ShlSigned64(dataWidth))

    val r_req            = Reg(new ShlSigned64RequestBundle(dataWidth))
...
class ShlSigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new ShlSigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new ShlSigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new ShlSigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new ShlSigned64ResponseBundle(dataWidth))
