...
class LtUnsigned64(val width: Int = 64) extends Module {
...
class LtUnsigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = UInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class LtUnsigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = Bool()
}


class LtUnsigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new LtUnsigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new LtUnsigned64ResponseBundle(dataWidth)))
}


class LtUnsigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new LtUnsigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new LtUnsigned64ResponseBundle(dataWidth)))
  val ip      = new LtUnsigned64IO(dataWidth)
}


class LtUnsigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new LtUnsigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new LtUnsigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new LtUnsigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new LtUnsigned64ResponseBundle(dataWidth)))
...
class LtUnsigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new LtUnsigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new LtUnsigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new LtUnsigned64(dataWidth))

    val r_req            = Reg(new LtUnsigned64RequestBundle(dataWidth))
...
class LtUnsigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new LtUnsigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new LtUnsigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new LtUnsigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new LtUnsigned64ResponseBundle(dataWidth))
  ...
  val LtUnsigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(Bool())

  switch(LtUnsigned64CP) {
    is(0.U) {
