...
class NeUnsigned64(val width: Int = 64) extends Module {
...
class NeUnsigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = UInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class NeUnsigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = Bool()
}


class NeUnsigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new NeUnsigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new NeUnsigned64ResponseBundle(dataWidth)))
}


class NeUnsigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new NeUnsigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new NeUnsigned64ResponseBundle(dataWidth)))
  val ip      = new NeUnsigned64IO(dataWidth)
}


class NeUnsigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new NeUnsigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new NeUnsigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new NeUnsigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new NeUnsigned64ResponseBundle(dataWidth)))
...
class NeUnsigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new NeUnsigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new NeUnsigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new NeUnsigned64(dataWidth))

    val r_req            = Reg(new NeUnsigned64RequestBundle(dataWidth))
...
class NeUnsigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new NeUnsigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new NeUnsigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new NeUnsigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new NeUnsigned64ResponseBundle(dataWidth))
  ...
  val NeUnsigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(Bool())

  switch(NeUnsigned64CP) {
    is(0.U) {
