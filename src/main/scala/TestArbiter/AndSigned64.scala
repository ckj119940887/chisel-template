...
class AndSigned64(val width: Int = 64) extends Module {
...
class AndSigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = SInt(dataWidth.W)
  val b = SInt(dataWidth.W)
}


class AndSigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = SInt(dataWidth.W)
}


class AndSigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new AndSigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new AndSigned64ResponseBundle(dataWidth)))
}


class AndSigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new AndSigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new AndSigned64ResponseBundle(dataWidth)))
  val ip      = new AndSigned64IO(dataWidth)
}


class AndSigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new AndSigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new AndSigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new AndSigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new AndSigned64ResponseBundle(dataWidth)))
...
class AndSigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new AndSigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new AndSigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new AndSigned64(dataWidth))

    val r_req            = Reg(new AndSigned64RequestBundle(dataWidth))
...
class AndSigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new AndSigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new AndSigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new AndSigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new AndSigned64ResponseBundle(dataWidth))
  ...
  val AndSigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(SInt(dataWidth.W))

  switch(AndSigned64CP) {
    is(0.U) {
