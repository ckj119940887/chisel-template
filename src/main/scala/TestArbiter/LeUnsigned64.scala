...
class LeUnsigned64(val width: Int = 64) extends Module {
...
class LeUnsigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = UInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class LeUnsigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = Bool()
}


class LeUnsigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new LeUnsigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new LeUnsigned64ResponseBundle(dataWidth)))
}


class LeUnsigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new LeUnsigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new LeUnsigned64ResponseBundle(dataWidth)))
  val ip      = new LeUnsigned64IO(dataWidth)
}


class LeUnsigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new LeUnsigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new LeUnsigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new LeUnsigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new LeUnsigned64ResponseBundle(dataWidth)))
...
class LeUnsigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new LeUnsigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new LeUnsigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new LeUnsigned64(dataWidth))

    val r_req            = Reg(new LeUnsigned64RequestBundle(dataWidth))
...
class LeUnsigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new LeUnsigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new LeUnsigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new LeUnsigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new LeUnsigned64ResponseBundle(dataWidth))
  ...
  val LeUnsigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(Bool())

  switch(LeUnsigned64CP) {
    is(0.U) {
