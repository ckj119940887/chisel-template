...
class AndUnsigned64(val width: Int = 64) extends Module {
...
class AndUnsigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = UInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class AndUnsigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = UInt(dataWidth.W)
}


class AndUnsigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new AndUnsigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new AndUnsigned64ResponseBundle(dataWidth)))
}


class AndUnsigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new AndUnsigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new AndUnsigned64ResponseBundle(dataWidth)))
  val ip      = new AndUnsigned64IO(dataWidth)
}


class AndUnsigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new AndUnsigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new AndUnsigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new AndUnsigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new AndUnsigned64ResponseBundle(dataWidth)))
...
class AndUnsigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new AndUnsigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new AndUnsigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new AndUnsigned64(dataWidth))

    val r_req            = Reg(new AndUnsigned64RequestBundle(dataWidth))
...
class AndUnsigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new AndUnsigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new AndUnsigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new AndUnsigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new AndUnsigned64ResponseBundle(dataWidth))
  ...
  val AndUnsigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(UInt(dataWidth.W))

  switch(AndUnsigned64CP) {
    is(0.U) {
