...
class LeSigned64(val width: Int = 64) extends Module {
...
class LeSigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = SInt(dataWidth.W)
  val b = SInt(dataWidth.W)
}


class LeSigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = Bool()
}


class LeSigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new LeSigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new LeSigned64ResponseBundle(dataWidth)))
}


class LeSigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new LeSigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new LeSigned64ResponseBundle(dataWidth)))
  val ip      = new LeSigned64IO(dataWidth)
}


class LeSigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new LeSigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new LeSigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new LeSigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new LeSigned64ResponseBundle(dataWidth)))
...
class LeSigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new LeSigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new LeSigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new LeSigned64(dataWidth))

    val r_req            = Reg(new LeSigned64RequestBundle(dataWidth))
...
class LeSigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new LeSigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new LeSigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new LeSigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new LeSigned64ResponseBundle(dataWidth))
  ...
  val LeSigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(Bool())
