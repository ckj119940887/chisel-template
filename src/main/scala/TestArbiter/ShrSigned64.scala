...
class ShrSigned64(val width: Int = 64) extends Module {
    ...
    val aU   = RegNext(io.a)
    val sign = RegNext(io.a(width-1) === 1.U)
    val top  = RegNext(Mux(sign, (-1).S(64.W), 0.S(64.W)))
...
class ShrSigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = SInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class ShrSigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = SInt(dataWidth.W)
}


class ShrSigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new ShrSigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new ShrSigned64ResponseBundle(dataWidth)))
}


class ShrSigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new ShrSigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new ShrSigned64ResponseBundle(dataWidth)))
  val ip      = new ShrSigned64IO(dataWidth)
}


class ShrSigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new ShrSigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new ShrSigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new ShrSigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new ShrSigned64ResponseBundle(dataWidth)))
...
class ShrSigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new ShrSigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new ShrSigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new ShrSigned64(dataWidth))

    val r_req            = Reg(new ShrSigned64RequestBundle(dataWidth))
...
class ShrSigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new ShrSigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new ShrSigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new ShrSigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new ShrSigned64ResponseBundle(dataWidth))
  ...
  val ShrSigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(SInt(dataWidth.W))

  switch(ShrSigned64CP) {
    is(0.U) {
