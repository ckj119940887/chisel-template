...
class RemainerUnsigned64(val width: Int = 64) extends Module {
...
class RemainerUnsigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = UInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class RemainerUnsigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = UInt(dataWidth.W)
}


class RemainerUnsigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new RemainerUnsigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new RemainerUnsigned64ResponseBundle(dataWidth)))
}


class RemainerUnsigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new RemainerUnsigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new RemainerUnsigned64ResponseBundle(dataWidth)))
  val ip      = new RemainerUnsigned64IO(dataWidth)
}


class RemainerUnsigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new RemainerUnsigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new RemainerUnsigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new RemainerUnsigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new RemainerUnsigned64ResponseBundle(dataWidth)))
...
class RemainerUnsigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new RemainerUnsigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new RemainerUnsigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new RemainerUnsigned64(dataWidth))

    val r_req            = Reg(new RemainerUnsigned64RequestBundle(dataWidth))
...
class RemainerUnsigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new RemainerUnsigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new RemainerUnsigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new RemainerUnsigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new RemainerUnsigned64ResponseBundle(dataWidth))
  ...
  val RemainerUnsigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(UInt(dataWidth.W))

  switch(RemainerUnsigned64CP) {
    is(0.U) {
