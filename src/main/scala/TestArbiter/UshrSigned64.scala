...
class UshrSigned64(val width: Int = 64) extends Module {
...
class UshrSigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = SInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class UshrSigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = SInt(dataWidth.W)
}


class UshrSigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new UshrSigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new UshrSigned64ResponseBundle(dataWidth)))
}


class UshrSigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new UshrSigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new UshrSigned64ResponseBundle(dataWidth)))
  val ip      = new UshrSigned64IO(dataWidth)
}


class UshrSigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new UshrSigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new UshrSigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new UshrSigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new UshrSigned64ResponseBundle(dataWidth)))
...
class UshrSigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new UshrSigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new UshrSigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new UshrSigned64(dataWidth))

    val r_req            = Reg(new UshrSigned64RequestBundle(dataWidth))
...
class UshrSigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new UshrSigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new UshrSigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new UshrSigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new UshrSigned64ResponseBundle(dataWidth))
  ...
  val UshrSigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(SInt(dataWidth.W))

  switch(UshrSigned64CP) {
    is(0.U) {
