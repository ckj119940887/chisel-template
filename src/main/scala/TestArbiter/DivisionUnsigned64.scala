...
class DivisionUnsigned64(val width: Int = 64) extends Module {
  val io = IO(new Bundle {
  ...
  val div = Module(new XilinxDividerUnsigned64Wrapper)
...
class DivisionUnsigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = UInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class DivisionUnsigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = UInt(dataWidth.W)
}


class DivisionUnsigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new DivisionUnsigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new DivisionUnsigned64ResponseBundle(dataWidth)))
}


class DivisionUnsigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new DivisionUnsigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new DivisionUnsigned64ResponseBundle(dataWidth)))
  val ip      = new DivisionUnsigned64IO(dataWidth)
}


class DivisionUnsigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new DivisionUnsigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new DivisionUnsigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new DivisionUnsigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new DivisionUnsigned64ResponseBundle(dataWidth)))
...
class DivisionUnsigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new DivisionUnsigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new DivisionUnsigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new DivisionUnsigned64(dataWidth))

    val r_req            = Reg(new DivisionUnsigned64RequestBundle(dataWidth))
...
class DivisionUnsigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new DivisionUnsigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new DivisionUnsigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new DivisionUnsigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new DivisionUnsigned64ResponseBundle(dataWidth))
  ...
  val DivisionUnsigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(UInt(dataWidth.W))

  switch(DivisionUnsigned64CP) {
    is(0.U) {
