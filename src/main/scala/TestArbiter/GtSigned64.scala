...
class GtSigned64(val width: Int = 64) extends Module {
...
class GtSigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = SInt(dataWidth.W)
  val b = SInt(dataWidth.W)
}


class GtSigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = Bool()
}


class GtSigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new GtSigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new GtSigned64ResponseBundle(dataWidth)))
}


class GtSigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new GtSigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new GtSigned64ResponseBundle(dataWidth)))
  val ip      = new GtSigned64IO(dataWidth)
}


class GtSigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new GtSigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new GtSigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new GtSigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new GtSigned64ResponseBundle(dataWidth)))
...
class GtSigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new GtSigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new GtSigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new GtSigned64(dataWidth))

    val r_req            = Reg(new GtSigned64RequestBundle(dataWidth))
...
class GtSigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new GtSigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new GtSigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new GtSigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new GtSigned64ResponseBundle(dataWidth))
  ...
  val GtSigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(Bool())
