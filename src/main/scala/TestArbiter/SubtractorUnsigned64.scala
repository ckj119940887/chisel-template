...
class SubtractorUnsigned64(val width: Int = 64) extends Module {
...
class SubtractorUnsigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = UInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class SubtractorUnsigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = UInt(dataWidth.W)
}


class SubtractorUnsigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new SubtractorUnsigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new SubtractorUnsigned64ResponseBundle(dataWidth)))
}


class SubtractorUnsigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new SubtractorUnsigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new SubtractorUnsigned64ResponseBundle(dataWidth)))
  val ip      = new SubtractorUnsigned64IO(dataWidth)
}


class SubtractorUnsigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new SubtractorUnsigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new SubtractorUnsigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new SubtractorUnsigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new SubtractorUnsigned64ResponseBundle(dataWidth)))
...
class SubtractorUnsigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new SubtractorUnsigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new SubtractorUnsigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new SubtractorUnsigned64(dataWidth))

    val r_req            = Reg(new SubtractorUnsigned64RequestBundle(dataWidth))
...
class SubtractorUnsigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new SubtractorUnsigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new SubtractorUnsigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new SubtractorUnsigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new SubtractorUnsigned64ResponseBundle(dataWidth))
  ...
  val SubtractorUnsigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(UInt(dataWidth.W))

  switch(SubtractorUnsigned64CP) {
    is(0.U) {
