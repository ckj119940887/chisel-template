...
class ShrUnsigned64(val width: Int = 64) extends Module {
...
class ShrUnsigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = UInt(dataWidth.W)
  val b = UInt(dataWidth.W)
}


class ShrUnsigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = UInt(dataWidth.W)
}


class ShrUnsigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new ShrUnsigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new ShrUnsigned64ResponseBundle(dataWidth)))
}


class ShrUnsigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new ShrUnsigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new ShrUnsigned64ResponseBundle(dataWidth)))
  val ip      = new ShrUnsigned64IO(dataWidth)
}


class ShrUnsigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new ShrUnsigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new ShrUnsigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new ShrUnsigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new ShrUnsigned64ResponseBundle(dataWidth)))
...
class ShrUnsigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new ShrUnsigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new ShrUnsigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new ShrUnsigned64(dataWidth))

    val r_req            = Reg(new ShrUnsigned64RequestBundle(dataWidth))
...
class ShrUnsigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new ShrUnsigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new ShrUnsigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new ShrUnsigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new ShrUnsigned64ResponseBundle(dataWidth))
  ...
  val ShrUnsigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(UInt(dataWidth.W))

  switch(ShrUnsigned64CP) {
    is(0.U) {
