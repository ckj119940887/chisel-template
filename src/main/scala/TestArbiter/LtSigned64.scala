...
class LtSigned64(val width: Int = 64) extends Module {
...
class LtSigned64RequestBundle(dataWidth: Int) extends Bundle {
  val a = SInt(dataWidth.W)
  val b = SInt(dataWidth.W)
}


class LtSigned64ResponseBundle(dataWidth: Int) extends Bundle {
  val out = Bool()
}


class LtSigned64IO(dataWidth: Int) extends Bundle {
  val req = Valid(new LtSigned64RequestBundle(dataWidth))
  val resp = Flipped(Valid(new LtSigned64ResponseBundle(dataWidth)))
}


class LtSigned64ArbiterIO(numIPs: Int, dataWidth: Int) extends Bundle {
  val ipReqs  = Flipped(Vec(numIPs, Valid(new LtSigned64RequestBundle(dataWidth))))
  val ipResps = Vec(numIPs, Valid(new LtSigned64ResponseBundle(dataWidth)))
  val ip      = new LtSigned64IO(dataWidth)
}


class LtSigned64ArbiterModule(numIPs: Int, dataWidth: Int) extends Module {
  val io = IO(new LtSigned64ArbiterIO(numIPs, dataWidth))
  ...
  val r_ipReq_bits = Reg(Vec(numIPs, new LtSigned64RequestBundle(dataWidth)))
  ...
  val r_reqBits  = Reg(new LtSigned64RequestBundle(dataWidth))
  ...
  val r_ipResp_bits  = Reg(Vec(numIPs, new LtSigned64ResponseBundle(dataWidth)))
...
class LtSigned64Wrapper(val dataWidth: Int ) extends Module {
    val io = IO(new Bundle{
        val req = Input(Valid(new LtSigned64RequestBundle(dataWidth)))
        val resp = Output(Valid(new LtSigned64ResponseBundle(dataWidth)))

    })

    val mod = Module(new LtSigned64(dataWidth))

    val r_req            = Reg(new LtSigned64RequestBundle(dataWidth))
...
class LtSigned64FunctionModule(dataWidth: Int) extends Module{
  val io = IO(new Bundle{
    val arb_req  = Valid(new LtSigned64RequestBundle(dataWidth))
    val arb_resp = Flipped(Valid(new LtSigned64ResponseBundle(dataWidth)))
  })

  val r_arb_req          = Reg(new LtSigned64RequestBundle(dataWidth))
  val r_arb_req_valid    = RegInit(false.B)
  val r_arb_resp         = Reg(new LtSigned64ResponseBundle(dataWidth))
  ...
  val LtSigned64CP            = RegInit(0.U(4.W))
  val r_res            = Reg(Bool())
